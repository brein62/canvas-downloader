...
class File:
  ...
  A file contains members `uuid`, `modified_at`, `id`, `url` and `display_name`.
  """

  def __init__(self, uuid : str, modified_at : str, id : int = None, url : str = None, display_name : str = None):
    """Creates a File object instance.

    Args:
      uuid (str): The UUID of the file, from the Canvas API.
  ...
  def fromApiArray(cls, apiArray : list[dict]) -> list[Self]:
    ...
    `uuid`, `modified_at`, `id`, `url` and `display_name`.
  ...
  def toLoadFileStr(self) -> str:
    ...
    in the locally-stored file log (`.files`), in the format "{uuid} {modified_at}\\n".

    Returns:
      str: The string representation of the File object.
    """
    return f'{self.uuid} {self.modified_at}\n'
  ...
  def __str__(self) -> str:
    ...
    Returns:
      str: The string representation of the File object.
    """
    return f'File with UUID {self.uuid}, Modified at {self.modified_at}, ID {self.id}, URL {self.url}, Display name {self.display_name}'
  ...
  def __repr__(self) -> str:
    ...
    Returns:
      str: The string representation of the File object.
    """
    return f'File({self.uuid}, {self.modified_at}, {self.id}, {self.url}, {self.display_name})'
...
class Folder:
  ...
  def __str__(self) -> str:
    ...
    """
    return f'Folder with ID {self.id}, path {self.full_name}, and files {self.files}'
...
class Course:
  ...
  """
  def __init__(self, id : int, name : str, course_code : str, folders : list[Folder] = None):
...
class FileLog:
  ...
  def fromFileLog(cls, fileLogLocation : str) -> Self:
    ...
    try:
      ...
      for line in loadedLines:
        uuid, modified_at = line.strip().split(" ")[:2]
        loadedFiles.append(File(uuid = uuid, modified_at = modified_at))
  ...
  def saveToFileLog(self, fileLogLocation : str):
    ...
    as a line with the format "{uuid} {modified_at}\\n".
  ...
  def findByUuid(self, uuid : str) -> File:
    """Finds the first file in the file log with a specified UUID. Note that UUID
    is unique, so the first file should be the first file desired.

    Args:
      uuid (str): The UUID of the file to find within the file log.
  ...
  def append(self, uuid : str, modified_at : str):
    """Appends a new File to the file log given its `uuid` and `modified_at`
    parameters.

    Args:
      uuid (str): The UUID of the new file to append to the file log.
      modified_at (str): The modified at timestamp of the new file to append to the file log.
    """
    self.fileList.append(File(uuid, modified_at))

  def update(self, uuid : str, modified_at : str):
    """Updates the modified at timestamp of the file in the file log, identified by UUID. If
    the file is not present in the file log, appends the file into the file log.

    Args:
      uuid (str): The UUID of the file in the file log to update the modified at timestamp.
      modified_at (str): The new modified at timestamp.
    """
    fileInLog = self.findByUuid(uuid)

    if fileInLog is None:
      self.append(uuid, modified_at)
    else:
  ...
  def isUpdated(self, file : File) -> bool:
    ...
    """
    loadedFile = self.findByUuid(file.uuid)
